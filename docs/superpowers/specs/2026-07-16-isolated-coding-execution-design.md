...
spawn 边界的固定身份策略如下：Linux、Android 与支持 descriptor-backed exec 的 Unix 从已打开的 executable fd 经 `/proc/self/fd` 或 `/dev/fd` 执行；macOS 是兼容性例外，因为 Darwin 不能可靠执行该 `/dev/fd` 路径，所以 supervisor 在全局 spawn lock 内完成 `PinnedExecutable` 身份与摘要重验后使用其 canonical pinned path，同时把同一已打开 handle 保留至 path-based exec，并让该 handle 保持 `CLOEXEC`。Darwin 不提供本项目可用的 fd-exec 原语，因此 macOS 无法获得 descriptor-backed 平台同等的“重验到 exec”原子性；由当前 OS 用户进程在这段极短窗口替换工具路径属于第 2.3 节明确排除的恶意当前用户代码边界。所有 Unix 仍在 `pre_exec` 中对已打开的 worktree fd 执行 `fchdir`。Windows 对 executable 持有 deny-write/deny-delete lease，并在 suspended `CreateProcess` 消费 cwd 路径期间持有已重验身份的目录 lease。Cargo 的 `RUSTC`、`RUSTDOC` 与可离线调用的 Git 都来自同一组 pinned tool handles，且在每次 Cargo spawn 前一并重验。直接或未知来源的 bootstrap `rustc --print sysroot` 也通过同一个 supervisor，而不是另走只终止 leader 的临时进程路径；对于严格 canonical、逐级 containment 校验且与 rustup 默认 concrete compiler 完全匹配的 `<RUSTUP_HOME>/toolchains/<default>`，直接采用该受管根作为 sysroot，避免 macOS descriptor-backed exec 破坏 compiler 自定位。
