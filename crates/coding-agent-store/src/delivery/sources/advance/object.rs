...
use super::super::model::{AdvanceDeliverySourceObjectRequest, DeliverySourceTransitionOutcome};
...
use super::super::validate::{
    ...
    validate_object_proof, validate_replay_anchor,
};
use super::verify::{
...
impl Store {
    pub async fn advance_delivery_source_object(
        &self,
        request: AdvanceDeliverySourceObjectRequest,
        ...
        .await?
        {
            TransitionLookup::Exact(receipt) => {
                ...
                if validate_object_proof(&source, &request.proof).is_err() {
                    transaction.commit().await?;
                    return Ok(DeliverySourceTransitionOutcome::Conflict);
                }
                transaction.commit().await?;
        ...
        let updated = sqlx::query(
            "UPDATE task_delivery_sources \
             SET expected_source_commit_oid = ?, state = 'commit_pending', failure_code = NULL, \
                 version = ?, updated_at = ? \
             WHERE task_id = ? AND repository_id = ? AND attempt = ? \
               AND state = 'object_pending' AND version = ? \
               AND expected_source_commit_oid IS NULL",
        )
        .bind(request.proof.expected_source_commit.as_str())
