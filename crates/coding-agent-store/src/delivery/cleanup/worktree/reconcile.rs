use crate::delivery::{
    ...
    ReconcileWorktreeCleanupRequest, WorktreeDisposition,
};
...
impl Store {
    pub async fn reconcile_worktree_cleanup(
        &self,
        request: ReconcileWorktreeCleanupRequest,
...
fn expected_worktree_fact(state: CleanupOperationState) -> WorktreeDisposition {
    match state {
        ...
        CleanupOperationState::UnlockedPendingRemove | CleanupOperationState::RemovePending => {
            WorktreeDisposition::RetainedUnlocked
        }
        _ => unreachable!("request constructor restricts reconciliation states"),
    }
}
